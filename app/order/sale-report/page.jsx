...
export default function OrderSaleReportPage() {
  ...
  // Fetch API data from admin/order/sale-report
  const loadSaleReport = useCallback(async (fDate = activeFromDate, tDate = activeToDate) => {
    setLoading(true);
    try {
      const res = await api.getOrderSaleReport({
        fromdate: fDate,
        todate: tDate,
      });
      ...
      console.error("Failed to load order sale report:", err);
      setDateGroups([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Order_Sale_Report_${activeFromDate}_to_${activeToDate}.csv`
    );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-500 mt-1">
              ...
              <Link href="/order" className="text-emerald-600 hover:text-emerald-700 font-medium">
                Orders
              </Link>
              <span className="text-slate-400">&gt;</span>
