...
export const api = {
  ...
  async getBarcodeList(payload) {
    try {
      const endpoint = `${API_BASE_URL}/admin/products/barcode`;
      const { ok, data } = await authFetch(endpoint, {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify({
          fromSku: payload.fromSku ? (isNaN(Number(payload.fromSku)) ? String(payload.fromSku).trim() : Number(payload.fromSku)) : 0,
          toSku: payload.toSku ? (isNaN(Number(payload.toSku)) ? String(payload.toSku).trim() : Number(payload.toSku)) : (payload.fromSku ? (isNaN(Number(payload.fromSku)) ? String(payload.fromSku).trim() : Number(payload.fromSku)) : 0),
          qty: Number(payload.qty || payload.quantity) || 1,
        }),
      });
      if (ok && data) {
        if (data.status === "success" && data.data?.barcodeList) {
          return data.data.barcodeList;
        }
        if (data.data?.barcodeList) return data.data.barcodeList;
        if (data.barcodeList) return data.barcodeList;
        if (Array.isArray(data.data)) return data.data;
        if (Array.isArray(data)) return data;
      }
      return [];
    } catch (e) {
      console.error("getBarcodeList API Error:", e);
      return [];
    }
  },
  ...
  async getStockReportHunter(params = {}) {
    try {
      const page = Number(params?.page) || 1;
      const limit = Number(params?.limit) || 30;

      const endpoint = `${API_BASE_URL}/admin/stockreport_new`;
      const payload = {
        page,
        limit,
        per_page: limit,
      ...
      if (params?.category_id !== undefined && params?.category_id !== null && params?.category_id !== "all") {
        payload.category_id = Number(params.category_id) || params.category_id;
      }
      if (params?.search) {
        payload.search = String(params.search).trim();
      }
      if (params?.skuCode) {
        payload.skuCode = String(params.skuCode).trim();
      }

      // Merge any additional custom parameters
      if (typeof params === "object") {
        Object.keys(params).forEach((key) => {
          if (!["page", "limit", "category_id", "search", "skuCode"].includes(key)) {
      ...
      if (ok && data && (data.status === "success" || data.success || data.categories || data.summary || data.data)) {
        return data;
      }
  ...
  async getPosSaleReport({ fromdate = "", todate = "" } = {}) {
    try {
      const endpoint = `${API_BASE_URL}/admin/pos/sale-report`;
      const token = getAuthToken();
      ...
      return data || { status: false, products: [] };
    } catch (e) {
      console.error("getPosSaleReport API Error:", e);
      return { status: false, products: [] };
    }
  },

  async getOrderSaleReport({ fromdate = "", todate = "" } = {}) {
    try {
      const endpoint = `${API_BASE_URL}/admin/order/sale-report`;
      const token = getAuthToken();
      ...
      console.error("getOrderSaleReport API Error:", e);
      return { status: false, products: [] };
    }
  },
  ...
  async deleteProductVariant(variantId, productId = null) {
    try {
      const endpoint = `${API_BASE_URL}/admin/product_variant_delete`;
      const payload = {
        id: String(variantId),
      ...
      return data || { success: false, status: "error", message: "Failed to delete product variant" };
    } catch (e) {
      console.error("deleteProductVariant API Error:", e);
      return { success: false, status: "error", message: e.message || "Network error deleting product variant" };
    }
  },
  ...
  async updateOrder(params = {}) {
    try {
      const endpoint = `${API_BASE_URL}/admin/order/update`;
      const payload = {
        id: Number(params.id || params.orderId || params.order_id) || params.id || params.order_id,
        note: params.note !== undefined && params.note !== null ? String(params.note) : (params.notes || ""),
      ...
      return data || { status: "error", message: "Failed to update order" };
    } catch (e) {
      console.error("updateOrder API Error:", e);
      return { status: "error", message: e.message || "Failed to update order" };
    }
  },

  async updatePosOrder(params = {}) {
